...
const I18N_ASSETS_DIR = 'assets/i18n';
...
/**
 * Get the full path for an i18n translation file
 * @param {string} filename - The filename (e.g., 'en.json')
 * @returns {string} Full path to the file in the i18n directory
 */
function getI18nPath(filename) {
  return path.join(I18N_ASSETS_DIR, filename);
}
...
const DEFAULT_FILE_CONTENTS = {
  ...
  'systemSettings.json': {
    ...
    "alertLocation": "דגניה",
    "pollingInterval": 10,
    "supportedAlertCategories": supportedAlertCategories,
    "areas": [
      0,
      1,
      2,
  ...
  'hk4KeyMap.json': {
    "enabled": true,
    "sequences": {
      "1111": { "gongType": 2, "areas": [0], "volume": 100, "repeat": 1 }
    }
  }
};
...
function initializeDataFiles() {
  ...
  for (const filename of DYNAMIC_DATA_FILES) {
    ...
    let defaultContent = DEFAULT_FILE_CONTENTS[filename] || '[]';
