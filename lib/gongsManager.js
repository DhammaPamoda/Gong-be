...
const removeJobFromLists = (aJob) => {
  ...
  if (aJob.isManual) {
    ...
    if (coursesTimeArray && coursesTimeArray.length > 0) {
      ...
      const foundIndex = scheduledCourseJobsArray.findIndex((scheduledCourseJob) => scheduledCourseJob.time === aJob.time);
      if (foundIndex >= 0) {
...
const gongManagerCallBackFunc = (aJob, aFireDate, aExecutedSuccess) => {
  ...
  logger.gongsManager.info(
    `GongManager.CallBackFunc ; time : ${firedTime}; Was Successful : ${aExecutedSuccess}`,
    { jobs: aJob },
  );
};
...
const handleJsonCourseScheduleRecord = (aJsonCourseScheduleRecord, aCurrentTime = moment().startOf('day')) => {
  ...
  const { rawMinutes: courseStartDateUtcOffsetInMinutes } = utilsManager.getLocalOffsetDetails(courseStartDate._d);
  ...
  const startDateInterpolatedInMSec = courseStartDate.clone().subtract(courseStartDay, 'd').valueOf();
    ...
    course.timedGongsArray.forEach(/** @param {ManualGong} timedGongRecord */(timedGongRecord) => {
      ...
      if (course.isTest) {
        ...
        for (let hour = startHour; hour <= endHour; hour += 1) {
          const timedTestGong = timedGongRecord.cloneWhileAddingTime(hour * milSecInHr, courseStartDateUtcOffsetInMinutes);
      ...
      timedGongRecordArray.forEach(/** @param {ManualGong} multipliedTimedGongRecord */(multipliedTimedGongRecord) => {
        const timedComputedGong = multipliedTimedGongRecord.cloneWhileAddingTime(startDateInterpolatedInMSec, courseStartDateUtcOffsetInMinutes);
        ...
        if (!(isAfter || currentTimeInMSec <= timedComputedGong.time)
          || (exceptions && exceptions.some(
            (exceptionGong) => exceptionGong.getTotalTimeInMsec() === multipliedTimedGongRecord.time,
          ))
        ) {
...
/**
 * Safely read and parse a JSON file with error handling for corrupted/empty files.
 * If the file is empty, corrupted, or doesn't exist, returns the default value
 * and attempts to repair the file.
 *
 * @param {string} aDataContainerDataName - The name of the data file (without .json extension)
 * @returns {Array|Object} The parsed JSON or default empty array
 */
const readFileSyncAsParsedJson = (aDataContainerDataName) => {
  const filePath = `${gongsManagerInstance.dynamicJsonFilesPath}/${aDataContainerDataName}.json`;
  const defaultContent = dataPaths.DEFAULT_FILE_CONTENTS[`${aDataContainerDataName}.json`] || '[]';

  try {
    const jsonFromFile = fs.readFileSync(filePath, 'utf8');

    // Check for empty file
    if (!jsonFromFile || jsonFromFile.trim() === '') {
      logger.gongsManager.warn(
        `readFileSyncAsParsedJson: File ${aDataContainerDataName}.json is empty. Initializing with default content.`,
      );
      // Repair the file with default content
      try {
        fs.writeFileSync(filePath, defaultContent, 'utf8');
        logger.gongsManager.info(`readFileSyncAsParsedJson: Repaired ${aDataContainerDataName}.json with default content.`);
      } catch (writeErr) {
        logger.gongsManager.error(`readFileSyncAsParsedJson: Failed to repair ${aDataContainerDataName}.json`, { error: writeErr });
      }
    ...
    if (error.code === 'ENOENT') {
      logger.gongsManager.warn(
        `readFileSyncAsParsedJson: File ${aDataContainerDataName}.json not found. Creating with default content.`,
      );
    } else if (error instanceof SyntaxError) {
      // Handle JSON parse error (corrupted file)
      logger.gongsManager.error(
        `readFileSyncAsParsedJson: File ${aDataContainerDataName}.json contains invalid JSON. Resetting to default.`,
        { error },
      );
    } else {
      logger.gongsManager.error(
        `readFileSyncAsParsedJson: Error reading ${aDataContainerDataName}.json`,
        { error },
      );
    }

    // Attempt to repair the file with default content
    try {
      fs.writeFileSync(filePath, defaultContent, 'utf8');
      logger.gongsManager.info(`readFileSyncAsParsedJson: Created/repaired ${aDataContainerDataName}.json with default content.`);
    } catch (writeErr) {
      logger.gongsManager.error(`readFileSyncAsParsedJson: Failed to create/repair ${aDataContainerDataName}.json`, { error: writeErr });
    }
...
/**
 * Persist a data object to a JSON file using atomic write to prevent corruption.
 * Uses write-to-temp-then-rename strategy to ensure file integrity even if
 * the write operation is interrupted (e.g., power loss, crash).
 *
 * @param {string} aDataContainerDataName - The name of the data file (without .json extension)
 * @param {Object|Array} aJsonObject - The object to persist
 * @param {Function} [aOnResolve] - Optional callback on success
 * @param {Function} [aOnReject] - Optional callback on failure
 * @returns {Promise<boolean>}
 */
const persistDataObject = (aDataContainerDataName, aJsonObject, aOnResolve, aOnReject) => new Promise(
  (resolve, reject) => {
    const filePath = `${gongsManagerInstance.dynamicJsonFilesPath}/${aDataContainerDataName}.json`;
    const jsonContent = JSON.stringify(aJsonObject);

    // Use atomic write to prevent file corruption
    dataPaths.atomicWriteFile(filePath, jsonContent)
      .then(() => {
        if (aOnResolve) {
          aOnResolve();
        }
      ...
      .catch((err) => {
        logger.gongsManager.error(`gongsManager.js=>persistDataObject(${aDataContainerDataName})`
          + ' Error: Failed to save data file', { error: err });
        if (aOnReject) {
          aOnReject();
        }
        reject(err);
...
const persistScheduledCourses = (aOnResolve, aOnReject) => persistDataObject(
  'coursesSchedule',
  gongsManagerInstance.scheduledCoursesArray,
  aOnResolve,
  aOnReject,
);
...
const persistManualGongs = (aOnResolve, aOnReject) => {
  ...
  returnPromise.then(() => {
    returnPromise = persistDataObject('obsoleteManualGong', gongsManagerInstance.obsoleteManualGongArray, aOnResolve, aOnReject);
  });
...
const cleanScheduleCourses = () => {
  if (gongsManagerInstance.passedCoursesArray.length > 0) {
    logger.gongsManager.info(
      'Obsolete scheduled course that were archived are : \n',
...
const getGongsAsJobsArrayForInit = () => {
  ...
  logger.gongsManager.info(
    'Initial manual gongs added to the system:',
    {
      init: true,
  ...
  gongsManagerInstance.automaticGongsMap.forEach((aTimedGongsArray, aScheduledCourseId) => {
    ...
    const scheduledCourse = gongsManagerInstance.scheduledCoursesArray.find((aScheduledCourse) => aScheduledCourse.id === aScheduledCourseId);

    logger.gongsManager.info(
      'Initial automatic gongs added to the system for course %s:',
      {
        init: true,
...
class GongsManager {
  constructor() {
    this.dynamicJsonFilesPath = dataPaths.DATA_DIR;
  ...
  addManualGong(aJsonManualGong) {
    if (!aJsonManualGong || !aJsonManualGong.time || !aJsonManualGong.gong || !aJsonManualGong.gong.areas
      ...
      const errMsg = `GongsManager.addManualGong ERROR.Json argument is [partly] empty or invalid. ****Gong = ${strigifiedGong}`;
  ...
  removeManualGong(aJsonScheduledGong) {
    ...
    const gongTime = Number(aJsonScheduledGong.time);

    // Normalize time to match how gongs are stored (seconds zeroed via zeroSecondsInTime)
    const normalizedTime = moment(gongTime).seconds(0).valueOf();

    const foundIndex = this.manualGongArray.findIndex((manualGong) => manualGong.time === normalizedTime);
    if (foundIndex >= 0) {
      // Normal removal flow
      this.obsoleteManualGongArray.push(this.manualGongArray[foundIndex]);
      this.manualGongArray.splice(foundIndex, 1);
    } else {
      // Check if already in obsolete array (already executed/cancelled)
      const alreadyObsolete = this.obsoleteManualGongArray.some(
        (gong) => gong.time === normalizedTime,
      );
      if (alreadyObsolete) {
        logger.gongsManager.info(
          `GongsManager.removeManualGong: Gong already removed/executed for time = ${normalizedTime}`,
        );
        // Return success - gong is already gone, which is the desired state
        return Promise.resolve(true);
      }
      // Truly not found - log warning but still return success for idempotency
      logger.gongsManager.warn(
        `GongsManager.removeManualGong: Gong not found for time = ${normalizedTime}, treating as already removed`,
      );
      return Promise.resolve(true);
    }
    ...
    persistPromise.then(() => {
      activateGongListeners(normalizedTime, null, 'DELETE');
    });
  ...
  toggleGong(aJsonScheduledCourseGong) {
    ...
    const daysOffset = exception.day_number - startDay;
    const timeOfDay = moment.duration(exception.time).asMilliseconds();
    const gongTime = momentOfCourseStart.clone().add(daysOffset, 'd').add(timeOfDay, 'ms').valueOf();

    const foundGong = timedGongsArray.find((/* ManualGong */gong) => gong.time === gongTime);
    if (!foundGong) {
      if (gongTime < moment().valueOf()) {
        const exceptionExists = foundCourseSchedule.isExceptionExists(exception);
        if (!exceptionExists) {
          foundCourseSchedule.insertException(exception);
          return persistScheduledCourses();
        }
        if (foundCourseSchedule.removeException(exception)) {
          return persistScheduledCourses();
        }
        return Promise.reject(new Error('GongsManager.toggleGong Error : could not remove Exception for past gong'));
      }
  ...
  retrieveAutomaticGongs() {
    // Use safe JSON reading to handle corrupted/empty files
    const coursesSchedule = readFileSyncAsParsedJson('coursesSchedule');
