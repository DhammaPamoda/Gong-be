...
const playSoundGong = (aGongToPlay, relayAndSoundManagerInstance) => {
  ...
  let whatGongType = utilsManager.gongsMap.get(gongType) || utilsManager.gongsMap.get(0);
  if (!whatGongType && utilsManager.gongsMap.size > 0) {
    whatGongType = utilsManager.gongsMap.values().next().value;
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!whatGongType) {
      const errorMsg = `❌ Gong type ${gongType} not found and no default (0) available.`;
      console.error(errorMsg);
      settlePromise(new Error(errorMsg));
      return;
    }
...
class RelayAndSoundManager {
  ...
  playGongForJob = (aGongToPlayJob, aFireDate) => {
    if (oref.getEmergencyState() && !aGongToPlayJob.isManual) {
      logger.relayAndSoundManager.warn('Skipping scheduled gong due to ACTIVE EMERGENCY STATE', { job: aGongToPlayJob });
      console.log('⚠️ Skipping scheduled gong due to ACTIVE EMERGENCY STATE');
