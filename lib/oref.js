...
class OrefManager {
  ...
  updatePolling(settings) {
    const { runSecurityCheck, overrideOrefUrl } = settings;
    ...
    if (runSecurityCheck) {
      const activeUrl = overrideOrefUrl ? overrideOrefUrl : 'Official Oref API';
      logger.log('info', `[Oref] Starting polling
        URL ${activeUrl}
        every ${pollingIntervalSec} seconds 
        for location: ${settings.alertLocation}`
      );
  ...
  checkOref() {
    ...
    try {
      ...
      if (settings.overrideOrefUrl) {
        try {
          const url = require('url');
          const parsedUrl = new url.URL(settings.overrideOrefUrl);
          ...
          logger.log('error', `[Oref] Invalid alternative URL provided: ${settings.overrideOrefUrl}`);
        }
      }
