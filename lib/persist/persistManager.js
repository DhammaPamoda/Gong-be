...
async function addOrDeleteTopicToLanguageBundles(aTopicValueArray, aTopicName, aIsAdd = true) {
...
async function addOrDeleteGongToLanguageBundles(aGongName, aIsAdd = true) {
  return addOrDeleteTopicToLanguageBundles([aGongName], 'gongType', aIsAdd);
}
...
function validateLanguagesTemplate(aUpdatedLanguagesJson) {
  ...
  if (aUpdatedLanguagesJson.some((langStructure) => !langStructure.language || !langStructure.translation)) {
...
function persistStaticData(
  aCourses4AddOrDelTemplateJson = null, aGong4Action = null, aUpdatedPermissions = null,
        ...
        if (error) {
          ...
          if (aCourses4AddOrDelTemplateJson) {
            if (aAction === actionEnum.ADD) {
              ...
              const removedObjectsArray = _.remove(currentStaticData.courses,
...
async function addOrDeleteCoursesToLanguageBundles(aCoursesTemplateJson, aIsAdd = true) {
  const newCoursesArray = Array.from(aCoursesTemplateJson);
  const coursesNamesArray = newCoursesArray.map((newCourse) => _.get(newCourse, 'course_name'));

  return addOrDeleteTopicToLanguageBundles(coursesNamesArray, 'courses', aIsAdd);
}
...
class PersistManager {
  ...
  async addCoursesTemplates(aNewCoursesTemplateJson) {
    try {
      ...
      await addOrDeleteCoursesToLanguageBundles(aNewCoursesTemplateJson);
  ...
  async deleteCoursesTemplates(aCourseName4Delete) {
    try {
      ...
      await addOrDeleteCoursesToLanguageBundles(coursesArrayLike, false);
