...
function validateCoursesTemplate(coursesTemplate, isNew) {
  if (!Array.isArray(coursesTemplate)) {
    throw new Error('Template must consist of an array of objects');
  }

  const coursesArray = Array.from(coursesTemplate);
  ...
  coursesArray.forEach((newCourse) => {
    ...
    if (isNew && existingCoursesNamesArray.includes(courseName)) {
    ...
    (resolve, reject) => {
      fs.readFile('assets/data/staticData.json', (error, data) => {
        if (error) {
          ...
          if (aGong4Action) {
            if (aAction === actionEnum.ADD) {
              const maxId = _.maxBy(currentStaticData.gongTypes, 'id')?.id || 0;
              _.set(aGong4Action, 'id', maxId + 1);
          ...
          if (aUpdatedCourseAgenda) {
            const courseName = _.get(aUpdatedCourseAgenda, 'course_name');
            const foundCourse = _.find(currentStaticData.courses, course => course.course_name === courseName);
            if (foundCourse) {
              foundCourse.course_agenda = _.get(aUpdatedCourseAgenda, 'course_agenda');
            } else {
              reject(new Error(
                `${errorPrefix} Failed to find Course for agenda update : ${courseName}`,
              ));
            }
          }
...
class PersistManager {
  ...
  async addCoursesTemplates(aNewCoursesTemplateJson) {
    try {
      validateCoursesTemplate(aNewCoursesTemplateJson, true);
  ...
  async updateCourseAgenda(aCourseAgendaJson) {
    try {
      if (!aCourseAgendaJson || !aCourseAgendaJson.course_name || !Array.isArray(aCourseAgendaJson.course_agenda)) {
        throw new Error('Course agenda update payload must contain course_name and course_agenda array');
      }
      validateCoursesTemplate([{
        course_name: aCourseAgendaJson.course_name,
        days_count: 10, // dummy for validation or ignored
        course_agenda: aCourseAgendaJson.course_agenda
      }], false);
      await persistStaticData(null, null, null, actionEnum.ADD, aCourseAgendaJson);
    } catch (e) {
      return Promise.reject(e);
    }
    return Promise.resolve();
