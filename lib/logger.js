...
const generalFormatter = printf(({ level, message, timeStamp, error }) => {
  let retValue = `${timeStamp} [${level}]: ${message}`;
  if (error) {
...
const reformatManualGongJobsForLog = (aJob, aExecTime) => {
  const formattedTime = moment(aJob.time).format('YY/MM/DD.HH:mm:ss');
  let formattedExecTimeClause = '';
  if (aExecTime) {
    const formattedExecTime = moment(aExecTime).format('YY/MM/DD.HH:mm:ss');
    const formattedPlayedTime = moment().format('YY/MM/DD.HH:mm:ss');
...
const reformatCourseForLog = (/* CourseSchedule */aCourseSchedule) => {
  ...
  const { exceptions } = aCourseSchedule;
...
logger.gongsManager = createLogger({
  ...
  transports: [
    new transports.File({
      filename: path.join(__dirname, '../logs/gongManager.log'),
      label: 'gongManager',
    }),
    new (transports.Console)({
      timestamp: tsFormat,
...
logger.scheduleManager = createLogger({
  ...
  transports: [
    new transports.File({
      filename: path.join(__dirname, '../logs/scheduleManager.log'),
...
logger.relayAndSoundManager = createLogger({
  ...
  transports: [
    new transports.File({
      filename: path.join(__dirname, '../logs/relayAndSoundManager.log'),
