...
if [[ "$PM2_PATH" != "pm2" ]] && [ -f "$PM2_PATH" ]; then
  sudo -S env "PATH=${PM2_DIR}:$PATH" "${PM2_PATH}" delete all 2>/dev/null || true <<<"${USER_PASS}"
  sudo -S env "PATH=${PM2_DIR}:$PATH" "${PM2_PATH}" kill 2>/dev/null || true <<<"${USER_PASS}"
else
  sudo -S env "PATH=$PATH" pm2 delete all 2>/dev/null || true <<<"${USER_PASS}"
  sudo -S env "PATH=$PATH" pm2 kill 2>/dev/null || true <<<"${USER_PASS}"
fi

if [ "${IS_DOCKER}" != "true" ]; then
  # Use absolute path to pm2 with sudo and ensure node is in PATH
  if [[ "$PM2_PATH" != "pm2" ]] && [ -f "$PM2_PATH" ]; then
    sudo -S env "PATH=${PM2_DIR}:$PATH" "${PM2_PATH}" start "/home/${USER}/projects/gong_dev_ops/dev_ops/gong_server_pm2_config.json" <<<"${USER_PASS}"
  else
    sudo -S env "PATH=$PATH" pm2 start "/home/${USER}/projects/gong_dev_ops/dev_ops/gong_server_pm2_config.json" <<<"${USER_PASS}"
  fi
else
  # This is relevant only when used in docker env
  if [[ "$PM2_RUNTIME_PATH" != "pm2-runtime" ]] && [ -f "$PM2_RUNTIME_PATH" ]; then
    sudo -S env "PATH=${PM2_DIR}:$PATH" "${PM2_RUNTIME_PATH}" "/home/${USER}/projects/gong_dev_ops/dev_ops/gong_server_pm2_config.json" <<<"${USER_PASS}"
  else
    sudo -S env "PATH=$PATH" pm2-runtime "/home/${USER}/projects/gong_dev_ops/dev_ops/gong_server_pm2_config.json" <<<"${USER_PASS}"
  fi
fi
...
if [[ "$PM2_PATH" != "pm2" ]] && [ -f "$PM2_PATH" ]; then
  sudo -S env "PATH=${PM2_DIR}:$PATH" "${PM2_PATH}" startup upstart -u root --hp "/home/${USER}" <<<"${USER_PASS}"
else
  sudo -S env "PATH=$PATH" pm2 startup upstart -u root --hp "/home/${USER}" <<<"${USER_PASS}"
fi

# Save pm2 configuration
if [[ "$PM2_PATH" != "pm2" ]] && [ -f "$PM2_PATH" ]; then
  sudo -S env "PATH=${PM2_DIR}:$PATH" "${PM2_PATH}" save <<< "${USER_PASS}"
else
  sudo -S env "PATH=$PATH" pm2 save <<< "${USER_PASS}"
fi
